...
#' @export 
ConleySE.matrix <- function(X, wgts, e, x, y, kernel, dist_fn, dist_cutoff, 
  lat_scale, verbose) {
...
ConleySE.lm <- function(model, x, y, 
  ...
  vcov <- ConleySE.matrix(X, wgts, e, x, y, kernel, dist_fn, dist_cutoff, lat_scale, verbose)
  return(vcov)
}

#' @export
ConleySE.plm <- function(model, x, y,
  id_var, time_var,
  kernel = "bartlett", dist_fn = "Haversine",
